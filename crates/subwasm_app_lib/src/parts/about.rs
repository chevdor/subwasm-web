...
impl super::App for About {
	fn name(&self) -> &'static str {
		"About Subwasm Web"
	}

	fn show(&mut self, ctx: &egui::Context, open: &mut bool) {
		egui::Window::new(self.name()).default_width(320.0).open(open).show(ctx, |ui| {
			use super::View as _;
			self.ui(ui);
		});
	}
...
fn about_things(ui: &mut egui::Ui) {
...
fn links(ui: &mut egui::Ui) {
	...
	ui.hyperlink_to(format!("{} Subwasm Web on GitHub", GITHUB), "https://github.com/chevdor/subwasm-web");
	ui.hyperlink_to(format!("{} Chevdor on Twitter", TWITTER), "https://twitter.com/chevdor");
}
